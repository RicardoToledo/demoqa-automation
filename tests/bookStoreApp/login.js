...
test ('Attempt to login with unregistered user', async t => {
    const unregisteredUser = userGenerator.generateLoginUser();
    await t.expect(await bookstore.submitLogin(unregisteredUser.userName, unregisteredUser.password)).notOk();
});